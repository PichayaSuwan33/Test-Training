...
`lookup` return index of given value in list.

```csharp
define variable cList as character no-undo.

cList = 'Value1;Value2;Value3':U.

message lookup('Value2':U, cList, ';':U).   /* 2 */
```

`entry` return value of given index in list.
...
message entry(1, cList, ';':U).   /* Value1 */

entry(1, cList, ';':U) = 'NewValue':U.

message cList.  /* NewValue;Value2;Value3 */
```

`can-do` return boolean is given value is in list. And also use to check pattern list.
...
message can-do(cList, 'Value1':U).    /* yes */

message can-do(cList, 'Value4':U).    /* no */
```

```csharp
define variable cPattern as character no-undo.

cPattern = '*@proalpha.com, *@gmail.com':U.

message can-do(cPattern, 'chasiree.katekaew@proalpha.com':U). /* yes */
```

> Read more info about `can-do`
> https://docs.progress.com/bundle/abl-reference/page/CAN-DO-function.html
